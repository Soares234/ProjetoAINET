    ...
    <form action="{{ action('SocioController@update', $user->id) }}" method="POST" class="form-group"
        ...
        @if($user->tipo_socio=="P")
            ...
            <div class="form-group">
                ...
                <select
                    ...
                    @foreach($tipos_licencas as $tipo_licenca)
                        <option {{old('tipo_licenca',$user->tipo_licenca)==$tipo_licenca->code?"selected":''}}
                                value='{{$tipo_licenca->code}}'>{{$tipo_licenca->nome}}</option>
                    @endforeach
            ...
            <div class="form-group">
                ...
                <select
                    ...
                    @foreach($classes_certificados as $classe_certificado)
                        <option {{old('classe_certificado',$user->classe_certificado)==$classe_certificado->code ? "selected" : ''}}
                            value='{{$classe_certificado->code}}'>{{$classe_certificado->code}}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group custom-control-inline">
                ...
                <input type="text" class="form-control" name="validade_certificado" id="validade_certificado"
                       value="{{Date('d/m/Y',strtotime($user->validade_certificado))}}"/>
            ...
            <div class="form-group custom-control-inline">
                ...
                <input type="text" class="form-control" name="validade_licenca" id="validade_licenca"
                       value="{{ Date('d/m/Y',strtotime($user->validade_licenca)) }}"/>
                <a class="form-control custom-control-inline btn btn-outline-primary text-center"
            ...
            <div class="form-group">
                ...
                <input type="file" class="form-control" name="file_certificado" id="certificado"
